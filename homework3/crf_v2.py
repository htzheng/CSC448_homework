...
def decode_batch(x, y, trans, emis, delta, pre, device='cpu'):
    B, N = x.size()
    T, W = emis.size()
    ...
    ''' decode batched '''
    delta = torch.zeros((B, N, T), dtype=torch.float32, device=device)
    delta.fill_(NINF)
...
def forward_backward_log_batch(x, y, trans, emis, log_a, log_b, device='cpu'):
    B, N = x.size()
    T, W = emis.size()
    ...
    for i in range(1, N):
        ...
        ''' faster version '''
        log_a[:,i,:] = (emis_logp[:,x_i].transpose(0,1).unsqueeze(1) + log_a[:,i-1,:].unsqueeze(2) + trans_logp.unsqueeze(0)).logsumexp(1) # 10x46                10x1x46                 10x46x1            1x46x46
...
if __name__ == '__main__':
    ...
    for iter, (x,y) in enumerate(sampler(train_x, train_y, batchsize=BATCH_SIZE)):
        ...
        if iter%200==0:
            with torch.no_grad():
                decode_idx, decode_accuracy = decode_batch(test_x, test_y, trans, emis, None, None, device=device)
                _, forward_backward_idx, forward_backward_accuracy = forward_backward_log_batch(test_x, test_y, trans, emis, None, None, device=device)
                print(f'iteration [{iter}/{MAX_ITER}], decode acc: [{decode_accuracy}], forward-backward acc: [{forward_backward_accuracy}]')
                # for bs in range(decode_idx.size(0)):
                #     print(decode_idx[bs,...])
                #     print(train_y[0:BATCH_SIZE][bs,...])
        
        optimizer.zero_grad()
        logp_marginal, _, _ = forward_backward_log_batch(x, y, trans, emis, None, None, device=device)
